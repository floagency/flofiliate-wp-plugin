...
require_once __DIR__ . '/FloFiliate_Manager.php';
...
class FloFiliate
{
	 ...
	 * @var      string
	 */
	protected $plugin_slug = 'FloFiliate';
	...
	private function __construct() {
        $this->manager = new FloFiliate_Manager();
	...
	private static function single_activate() {
		if(!class_exists('FloFilliate\Api')) {
            exit("Unable to locate FloFiliate Api library");
        }
	}
	...
	public function add_plugin_admin_menu() {
		...
		$this->plugin_screen_hook_suffix = add_options_page(
			__( 'FloFiliate Settings', $this->plugin_slug ),
			__( 'FloFiliate Settings', $this->plugin_slug ),
			'manage_options',
			$this->plugin_slug,
    ...
    /**
     * Register allowed settings
     *
     * @since    1.0.0
     */
    public function register_mysettings() {
        register_setting( 'flofiliate-group', 'flofiliate_api_url' );
    }
	...
	public function display_plugin_admin_page() {
        if(isset($_POST['__flofiliate_submit'])) {
            $api_url = isset($_POST['flofiliate_api_url']) ? $_POST['flofiliate_api_url'] : null;

            if(!empty($api_url) && filter_var($api_url, FILTER_VALIDATE_URL)) {
                if(get_option( 'flofiliate_api_url' ) !== false) {
                    update_option('flofiliate_api_url', $api_url);
                } else {
